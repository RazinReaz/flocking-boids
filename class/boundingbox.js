...
class BoundingBox {
    ...
    contains(point) {
        horizontally_inside = this.x < point.x && point.x < this.x + this.width;
        vertically_inside = this.y < point.y && point.y < this.y + this.height;
        return horizontally_inside && vertically_inside;
    }

    contains(point_x, point_y) {
        horizontally_inside = this.x < point_x && point_x < this.x + this.width;
        vertically_inside = this.y < point_y && point_y < this.y + this.height;
        return horizontally_inside && vertically_inside;
    ...
    intersects_with_circle(circle_x, circle_y, circle_radius) {
        ...
        return false;
        closest_rect_x = max(this.x, min(circle_x, this.x + this.width));
        closest_rect_y = max(this.y, min(circle_y, this.y + this.height));
